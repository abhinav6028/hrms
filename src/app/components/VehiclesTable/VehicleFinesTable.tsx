...
const VehicleFinesTable: FC<Prop> = () => {
  ...
  const columns: MRT_ColumnDef<VehicleFinesDetails>[]  = useMemo<MRT_ColumnDef<VehicleFinesDetails>[]>(
    ...
    //     columns: [
            {
                accessorFn: (row) => `${row.model}`, //accessorFn used to join multiple data into a single cell
    ...
    {    
        ...
        columns: [
            {
                accessorFn: (row) => new Date(row.mrDate), //convert to Date for sorting and filtering
                id: 'RenewalDate',
                header: 'Date',
                filterFn: 'dateFilter',
                sortingFn: 'datetime',
