...
export type RenewalDetails = {
    ...
    licenseRenewalDone?: boolean
    vehicleRenewal: string
    vehicleRenewalDone?: boolean
    avatar?: string
  };

const RenewalStatusTable = () => {
  const columns = useMemo<MRT_ColumnDef<RenewalDetails>[]>(
    () => [
    {
    ...
    {    
        ...
        muiTableHeadCellProps: {
            ...
            sx: {
                ...
                backgroundColor: '#C2AED6 !important',
                outline: '6px',
                outlineStyle: 'solid',
        ...
        columns: [
            {
                accessorFn: (row) => new Date(row.licenseRenewal), //convert to Date for sorting and filtering
            ...
            },
            {
                ...
                accessorFn: (row) => {
                        const date = new Date(row.licenseRenewal)
                        const currentDate = moment().format('MM-DD-YYYY');
                        const difference = moment(currentDate).diff(date, 'days');
                        if (row.licenseRenewalDone)
                            return ('1Completed')
    ...
    {    
        ...
        columns: [
            {    
                accessorFn: (row) => new Date(row.vehicleRenewal), //convert to Date for sorting and filtering
                id: 'vehicleRenewalDate',
                header: 'Date',
                filterFn: 'dateFilter',
                sortingFn: 'datetime',
                Cell: ({ cell }) => cell.getValue<Date>()?.toLocaleDateString(), //render Date as a string
            ...
            {
                id: 'vehicleRenewalStatus',
                header: 'Status',
                accessorFn: (row) => {
                    const date = new Date(row.vehicleRenewal)
                    const currentDate = moment().format('MM-DD-YYYY');
                    const difference = moment(currentDate).diff(date, 'days');
                    if (row.vehicleRenewalDone)
                        return ('1Completed')
                    else if(difference>0)
                        return (`2Overdue by ${difference} days`)
                    else if(difference<=0 && difference>=-30)
...
<h5 className="text-black-50 mb-10"><i className="bi bi-caret-right-fill fs-2 me-2 svg-icon-muted text-black-50"></i>  {/* icon  */} Employee Document Renewal Status</h5>
        ...
        <div className={`card`}>
            ...
            <h3 className='card-title align-items-start flex-column'>
                {/* <span className='card-label fw-bold fs-3 mb-1'>Employee Document Renewal Status</span> */}
